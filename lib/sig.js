 ...
 *
 * @param {Object} format
 * @param {vm2.VMScript} decipherScript
 * @param {vm2.VMScript} nTransformScript
 */
exports.setDownloadURL = (format, decipherScript, nTransformScript) => {
  const decipher = url => {
    ...
    const decipherVM = new vm.VM({ sandbox: { sig: decodeURIComponent(args.s) } })
    components.searchParams.set(args.sp ? args.sp : 'signature',
      decipherVM.run(decipherScript));
  ...
  const ncode = url => {
    ...
    const nTransformVM = new vm.VM({ sandbox: { ncode: n } })
    components.searchParams.set('n', nTransformVM.run(nTransformScript));
    return components.toString();
  };
...
exports.decipherFormats = async (formats, html5player, options) => {
  ...
  const decipherScript = functions.length ? new vm.VMScript(functions[0]) : null;
  const nTransformScript = functions.length > 1 ? new vm.VMScript(functions[1]) : null;
